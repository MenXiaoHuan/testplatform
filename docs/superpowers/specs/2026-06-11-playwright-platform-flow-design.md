...
- 报告页支持附件表格内逐条 `打开 / 下载`
- 报告页增加失败用例优先区，优先暴露 `FAILED` 结果
- 报告页支持基础状态筛选：`全部 / FAILED / PASSED / SKIPPED`
- 报告页增加按 `projectName` 的轻量统计汇总
- 上述筛选与统计优先由前端基于 `report-summary` 返回的 `caseResults` 完成，避免为第一阶段新增复杂聚合接口
...
- 保留现有“打开外部报告”能力
- 附件列表中的每条附件提供单独的 `打开 / 下载` 操作
- 默认展示全部用例结果
- 用户切换状态筛选时，仅前端过滤当前聚合结果，不触发新的后端请求
- `FAILED` 用例在独立失败优先区中先行展示
